...
When **ECOSYSTEM_CONTEXT** is injected (via `./install.sh` or `./install.sh install`), use it as live ground truth for this machine:
...
**Refresh:** `./install.sh`, `./install.sh detect --write`, or `./install.sh link`.
