...
Keeps prompts data-driven: generated when ``./install.sh install`` runs, not hand-edited tables.
"""
...
_GLOBAL_SNAPSHOT_OMIT = (
    ...
    "`./install.sh subagent` from the repo root "
    "(or `./install.sh install --target <repo> --tools cursor`).\n\n"
...
def build_target_cartography(target: Path) -> str:
    ...
    lines: list[str] = [
        ...
        "`./install.sh install --target .` to refresh.*",
        "",
    ]
